...
* リモートサポートは、必ずエンジニアと通話を維持した状態で進めてください。開始から終了まで、通話を切らずに維持する必要があります。
{% endhint %}

{% hint style="warning" %}
**ネットワーク環境のご確認**

* 画面のフリーズや通信エラーを防ぐため、お客様のネットワーク状態をご確認ください。
* セルラー（モバイル通信）や通信信号が安定した状態で実施することを推奨します。
{% endhint %}

{% hint style="danger" %}
**ご乗車の確認**

* 安全確保のため、リモートサポートは必ず車両に乗車した状態で行ってください。
{% endhint %}
...
{% hint style="warning" %}
**\[リモートサポートを終了する]**&#x3092;選択すると、直ちにコードは無効になります。この場合、再度コードを発行する必要があります。
{% endhint %}
{% endstep %}
