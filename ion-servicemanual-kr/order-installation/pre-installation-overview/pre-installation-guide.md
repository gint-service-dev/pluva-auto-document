...
아래 모델은 설치할 수 없습니다. 방문 전에 모델명을 반드시 확인합니다.

* 풀 크롤러(Full Crawler) 모델
* 핸들형 콤바인
{% endhint %}
{% endstep %}
...
* **이앙기 GNSS 거치대**: 이앙기에 설치할 때는 수신기 전용 거치대가 반드시 필요합니다. 고객이 방문 전에 미리 준비해야 하며, 거치대가 없으면 당일 수신기 설치가 어렵습니다.
  * 순정 거치대가 없다면, 흔들리지 않는 평평하고 단단한 구조물을 준비하도록 안내합니다.
...
* **사용 예정인 RTK 방식 확인**: 퀵셋업에서 위치 보정 방식을 설정합니다. 방문 전에 고객이 어떤 RTK를 사용할지 확인하고 고객에게 필요 정보를 확보합니다.
  * **Smart RTK**: 별도 계정 없이 자동으로 보정 신호를 수신하는 방식입니다. 사전에 준비할 계정 정보는 없으며, 원활한 연결을 위해 현장의 네트워크 환경을 미리 확인합니다.
