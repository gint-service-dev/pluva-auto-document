...
1. **지도 보기 전환** : \[경로] / \[RTK 품질] 보기를 전환합니다.
2. **방향** : 지도의 북쪽 방향을 표시합니다.
3. **현재 위치** : 지도를 작업 위치로 이동(중심 정렬)합니다.
4. **지도 경로** : 지도에 작업의 주행 궂적이 표시됩니다. 시작 지점(S)·종료 지점(E)과 색상·아이콘의 의미는 경로 범례에서 확인합니다.
5. **경로 범례** : 지도에 표시된 경로 색상·아이콘의 의미를 확인합니다.
6. **상세 정보** : 해당 작업의 요약 정보(작업 정보) 패널을 엽니다.
7. **실시간 데이터** : 재생 시점을 기준으로 RTK 품질, 속도 등 상세 수치를 표시합니다.
8. **재생 / 타임라인** : \[▶]로 작업을 재생하고, 타임라인에서 특정 시점으로 이동합니다.
9. **재생 속도** : 재생 배속(예: 1x)을 선택합니다.
