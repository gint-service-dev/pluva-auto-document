---
layout:
  width: default
  title:
    visible: true
  description:
    visible: false
  tableOfContents:
    visible: true
  outline:
    visible: true
  pagination:
    visible: true
  metadata:
    visible: true
  tags:
    visible: true
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/256Umh24fJVf6zNkZpSa/order-installation/installation-completed
---

# 取り付け完了確認

取り付け及び簡単セットアップの完了項目を確認し、取り付けチケットの状態を「取り付け完了」に変更する作業です。

{% hint style="info" %}
**全ての取り付け作業を完了してから\[確認完了]をタップします。**

* 製品が正常に取り付けられました。
* 必要なソフトウェアの設定がすべて完了しました。
* お客様がすぐにサービスを利用できる状態です。
{% endhint %}

### pluva ion（完成品）の場合

{% stepper %}
{% step %}
取り付けチケットリストから、取り付け完了チケットを選択します。

<figure><img src="../.gitbook/assets/installation-completion-1.png" alt="" width="224"><figcaption></figcaption></figure>
{% endstep %}

{% step %}
\[取り付け完了確認]をタップします。

<figure><img src="../.gitbook/assets/installation-completion-2.png" alt="" width="216"><figcaption></figcaption></figure>
{% endstep %}

{% step %}
取り付け完了確認の内容を確認し、\[確認完了]をタップします。

<figure><img src="../.gitbook/assets/installation-completion-3.png" alt="" width="211"><figcaption></figcaption></figure>
{% endstep %}

{% step %}
取り付けが完了されます。

<figure><img src="../.gitbook/assets/installation-completion-4.png" alt="" width="180"><figcaption></figcaption></figure>
{% endstep %}
{% endstepper %}

***

### pluva ion エクスパンションキットの場合

{% stepper %}
{% step %}
取り付けチケットリストから、取り付け完了チケットを選択します。

<figure><img src="../.gitbook/assets/installation-completion-expansion-1.png" alt="" width="224"><figcaption></figcaption></figure>
{% endstep %}

{% step %}
\[取り付け完了確認]をタップします。

<figure><img src="../.gitbook/assets/installation-completion-expansion-2.png" alt="" width="216"><figcaption></figcaption></figure>
{% endstep %}

{% step %}
製品を取り付けたお客様のアカウントを選択します。

<figure><img src="../.gitbook/assets/installation-completion-expansion-3.png" alt="" width="216"><figcaption></figcaption></figure>

{% hint style="info" %}
お客様名、または電話番号で検索すると、お客様リストが表示されます。リストが表示されない場合は、検索キーワードが正しく入力されているかご確認ください。

<img src="../.gitbook/assets/installation-completion-expansion-account.png" alt="" data-size="original">
{% endhint %}
{% endstep %}

{% step %}
取り付け完了確認の内容を確認し、\[確認完了]をタップします。

<figure><img src="../.gitbook/assets/installation-completion-expansion-4.png" alt="" width="216"><figcaption></figcaption></figure>
{% endstep %}

{% step %}
取り付けが完了されます。

<figure><img src="../.gitbook/assets/installation-completion-expansion-5.png" alt="" width="180"><figcaption></figcaption></figure>
{% endstep %}
{% endstepper %}
