...
1. タブレット正面のスリープボタンを押します。
2. タブレット画面が完全に消えるまでしばらく待ちます。
3. 画面が消えてから約10秒後、メイン電源スイッチをOFFにします。
{% endhint %}

{% hint style="warning" %}
メイン電源スイッチで直接電源を切ると、作業内容が保存されず、次のような問題が発生する恐れがあります。必ず、正しい手順でシャットダウンを行ってください。

* 直前に設定したABラインが保存されません。
* 保存した経路を再度取り込むことができません。
{% endhint %}
