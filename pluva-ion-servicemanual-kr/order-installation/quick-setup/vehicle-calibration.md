...
* 차량 보정을 취소하거나 보정에 실패한 경우에는 중간 단계부터 재개하지 말아주세요.
* 핸들 범위 보정으로 돌아가 왼쪽 끝과 오른쪽 끝을 다시 측정한 뒤 직진 조향 보정을 진행해야 합니다.
* 보정이 계속 진행되지 않으면 태블릿을 재부팅한 뒤 전체 차량 보정을 처음부터 다시 진행하세요.
{% endhint %}
