...
{% hint style="info" %}
상위 조직을 변경하면 조직 계층이 바뀌고, 해당 조직에 소속된 오퍼레이터의 접근 권한이 자동으로 재설정됩니다.

* 해당 조직과 모든 하위 조직이 새 상위 조직 아래로 이동합니다.
* 소속 오퍼레이터의 접근 범위가 새 계층 기준으로 재계산되며, 이전에 추가로 부여한 권한은 초기화됩니다. 필요하면 추가 권한을 다시 설정합니다.
{% endhint %}
