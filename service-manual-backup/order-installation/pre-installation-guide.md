...
* **RTK 서비스 계정**: 퀵셋업에서 위치 보정 방식을 설정합니다. 방문 전에 고객이 어떤 RTK를 사용할지 확인합니다.

  * **Smart RTK**: 별도 계정 없이 자동으로 보정 신호를 수신하는 방식입니다. 사전에 준비할 계정 정보는 없으며, 원활한 연결을 위해 현장의 네트워크 환경을 미리 확인합니다.

    {% hint style="info" %}
    **현장에서 연결이 안 될 경우 아래 항목을 먼저 확인합니다.**
    * 네트워크(LTE/Wi-Fi)가 연결되어 있나요?
    * GPS 신호가 수신되고 있나요?
    {% endhint %}

  * **고객 RTK(직접 수신 / 블루투스)**: 기지국·서버 주소, 포트, 마운트포인트, 계정 ID 및 비밀번호를 방문 전에 고객으로부터 확보합니다.
